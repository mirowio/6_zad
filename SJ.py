class SkipJack:
	def __init__(self):
		# F это 8-битный S-box
		self.F = []
		self.defineF()
		# w1, w2, w3, w4 это 16-битные переменные типа "int"
		self.w1 = 0
		self.w2 = 0
		self.w3 = 0
		self.w4 = 0
	
	
	# plaintext (преобразумое сообщения) - это 64-битная переменная типа "int"
	# key (ключ) - это список 10-байтников
	def encrypt(self, plaintext, key):
		self.splitWord(plaintext)

		for round in range (1, 33):
			if (1 <= round <= 8) or (17 <= round <= 24):
				self.A(round, key)
				#self.printStatus(round)
			if (9 <= round <= 16) or (25 <= round <= 32):
				self.B(round, key)
				#self.printStatus(round)

		return self.appendWords()


	# ciphertext(зашифрованное сообщение) это 64-битная переменная типа "int"
	# key (ключ) - это список 10-байтников
	def decrypt(self, ciphertext, key):
		self.splitWord(ciphertext)

		for round in reversed(range(1, 33)):
			if (25 <= round <= 32) or (9 <= round <= 16):
				self.Binv(round, key)
			if (17 <= round <= 24) or (1 <= round <= 8):
				self.Ainv(round, key)

		return self.appendWords()


	def A(self, round, key):
		c1 = self.w1
		c2 = self.w2
		c3 = self.w3
		self.w1 = self.G(round, key, c1) ^ self.w4 ^ round
		self.w2 = self.G(round, key, c1)
		self.w3 = c2
		self.w4 = c3


	def Ainv(self, round, key):
		c1 = self.w1
		c2 = self.w2
		self.w1 = self.Ginv(round, key, c2)
		self.w2 = self.w3
		self.w3 = self.w4
		self.w4 = c1 ^ c2 ^ round


	def B(self, round, key):
		c1 = self.w1
		c2 = self.w2
		c3 = self.w3
		self.w1 = self.w4
		self.w2 = self.G(round, key, c1)
		self.w3 = c1 ^ c2 ^ round
		self.w4 = c3


	def Binv(self, round, key):
		c1 = self.w1
		self.w1 = self.Ginv(round, key, self.w2)
		self.w2 = self.Ginv(round, key, self.w2) ^ self.w3 ^ round
		self.w3 = self.w4
		self.w4 = c1


	# w здесь это 16-битная переменная всё того же типа "int"
	def G(self, round, key, w):
		g = [0] * 6
		g[0] = (w >> 8 ) & 0xff
		g[1] = w & 0xff
		j = (4 * (round - 1)) % 10

		for i in range(2, 6):
			g[i] = self.F[g[i - 1] ^ key[j]] ^ g[i - 2]
			j = (j + 1) % 10

		return (g[4] << 8) | g[5] 


	# w здесь это 16-битная переменная всё того же типа "int"
	def Ginv(self, round, key, w):
		g = [0] * 6
		g[4] = (w >> 8) & 0xff
		g[5] = w & 0xff
		j = (4 * (round - 1) + 3) % 10

		for i in reversed(range(4)):
			g[i] = self.F[g[i + 1] ^ key[j]] ^ g[i + 2]
			j = (j - 1) % 10

		return (g[0] << 8) | g[1]


	# Эта функция соединяет четыре 16-битных слова w1,w2,w3,w4 в одно 64-битное
	def appendWords(self):
		x1 = self.w1 << 3 * 16
		x2 = self.w2 << 2 * 16
		x3 = self.w3 << 1 * 16
		x4 = self.w4
		return x1 | x2 | x3 | x4


	# w - это 64-битное слово. Функция разбивают эту переменную на четыре 16-битных слова w1, w2, w3, w4
	def splitWord(self, w):
		self.w1 = (w >> (16 * 3)) & 0xffff
		self.w2 = (w >> (16 * 2)) & 0xffff
		self.w3 = (w >> (16 * 1)) & 0xffff
		self.w4 = w & 0xffff


	# Выводит номер раунда и текущие значения w1,w2,w3,w4
	def printStatus(self, round):
		w = self.appendWords()
		print("round=" + str(round) + "  " + hex(w))


	def defineF(self):
		self.F = []
